...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    public void OnCreateRoomButtonClicked()
    {
        ...
        if (roomOptions.MaxPlayers > 10) //only ten max
        {
    ...
    {
        PhotonNetwork.LoadLevel("SceneMapLuzon");
    }
    ...
    public override void OnJoinedRoom()
    {
        ...
        foreach(Player player in PhotonNetwork.PlayerList)
        {
            ...
            playerItem.transform.Find("TextPlayerName").GetComponent<TextMeshProUGUI>().text = player.NickName;
            //if the player in the room is you, it turns on the indicator
            playerItem.transform.Find("ImagePlayerIndicator").gameObject.SetActive(player.ActorNumber == PhotonNetwork.LocalPlayer.ActorNumber);
    ...
    public override void OnRoomListUpdate(List<RoomInfo> roomList) 
    {
        ...
        foreach(RoomInfo y in cachedRoomList.Values)
        {
            ...
            listItem.transform.Find("TextRoomName").GetComponent<TextMeshProUGUI>().text = y.Name;
            listItem.transform.Find("TextRoomPlayers").GetComponent<TextMeshProUGUI>().text = "Player count: " + y.PlayerCount + "/" + y.MaxPlayers;
            listItem.transform.Find("ButtonJoinRoom").GetComponent<Button>().onClick.AddListener(() => OnJoinRoomClicked(y.Name));
